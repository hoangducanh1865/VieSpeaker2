...
class ASTP(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.global_context_att:
            ...
            #     x_32 = x.float()  # ép về FP32 để tính toán ổn định
            #     context_mean = torch.mean(x_32, dim=-1, keepdim=True).expand_as(x_32)
            #     context_std = torch.sqrt(torch.var(x_32, dim=-1, keepdim=True) + 1e-7).expand_as(x_32)
            #     x_in = torch.cat((x_32, context_mean, context_std), dim=1)
            #     x_in = x_in.to(x.dtype)  # đưa lại về dtype ban đầu (FP16) nếu cần
