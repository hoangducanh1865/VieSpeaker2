"""CAM++ (CAMPPlus) speaker-embedding architecture.

Self-contained copy of the 3D-Speaker / FunASR CAMPPlus model so the project does
not depend on the `modelscope`/`speakerlab` packages at inference time. Matches the
state dict published as `iic/speech_campplus_sv_zh-cn_16k-common`
(feat_dim=80, embedding_size=192).

Source: https://github.com/modelscope/3D-Speaker (Apache-2.0).
"""
        ...
        for i in range(num_layers):
            self.add_module(
                ...
                CAMDenseTDNNLayer(
                    ...
                    out_channels=out_channels, bn_channels=bn_channels, kernel_size=kernel_size,
...
class BasicResBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        super().__init__()
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=(stride, 1),
...
class FCM(nn.Module):
    def __init__(self, block=BasicResBlock, num_blocks=(2, 2), m_channels=32, feat_dim=80):
        ...
        self.conv2 = nn.Conv2d(m_channels, m_channels, kernel_size=3, stride=(2, 1),
                               padding=1, bias=False)
...
class CAMPPlus(nn.Module):
    def __init__(self, feat_dim=80, embedding_size=192, growth_rate=32, bn_size=4,
        ...
        self.xvector = nn.Sequential(OrderedDict([
            ('tdnn', TDNNLayer(channels, init_channels, 5, stride=2, dilation=1,
