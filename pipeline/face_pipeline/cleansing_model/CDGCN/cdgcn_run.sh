...
if [ $stage == "Train" ]; then
	...
	conda run -n cdgcn bash -c "export PYTHONPATH=. && OMP_NUM_THREADS=1 python tools/create_knn.py \
    --prefix $prefix \
    --name $name \
    --dim $dim \
	...
	if [ $gpu_num -gt 1 ]; then
		conda run -n cdgcn bash -c "export PYTHONPATH=. && NCCL_P2P_LEVEL=NVL python launch.py --use_env --nproc_per_node $gpu_num cdgcn/main.py   --phase train     --config $config --gpus $gpu_num --distributed"
	else
		#single-gpu
		conda run -n cdgcn bash -c "export PYTHONPATH=. && python cdgcn/main.py   --phase train     --config $config"
	fi
...
if [ $stage == "Test_2nd" ]; then
	...
	conda run -n cdgcn python cdgcn/diar_ovl_assignment_ms.py  $osd_dir
