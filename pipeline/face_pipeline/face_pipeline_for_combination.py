...
lr_asd_path = os.path.join(current_dir, 'audio_visual_model', 'LR-ASD')

# Insert LR-ASD path so its local modules (ASD.py, loss.py, model/) resolve.
if lr_asd_path not in sys.path:
...
class ASVDataPipeline:
    ...
    def phase_0_preprocess_fps(self, video_path):
        ...
        standardized_video_path = os.path.join(self.out_dir, f"{name}_from30s_25fps_hq.mp4")

        if os.path.exists(standardized_video_path):
            print(f"Standardized high-quality video already exists at: {standardized_video_path}")
            return standardized_video_path

        print("Discarding first 30 seconds and converting to strict 25 FPS with high quality...")
        command = [
    ...
    def phase_3_cluster_identities(self, distance_threshold=0.6):
        ...
        if len(features) == 1:
            ...
            clusterer = AgglomerativeClustering(
                n_clusters=None, 
                affinity='precomputed', # Fix bug không nhận diện được metric là tham số gì, có thể là do lỗi phiên bản thư viện khác nhau
                linkage='average', 
